...
public class Bank {
    ...
    //1. 매개변수로 전달되는 계좌번호과 회원이름과 동일한 정보를 가진
    // 고객 정보를 리턴하는 메서드를 정의하시오.
    // 만약 매개변수로 전달된 계좌번호와 일치하는 고객이 없다면 null을 리턴하시오.
    List<Client> clientInfo(Client client){
        for (int i = 0; i < clientList.size(); i++){
            if (clientList.get(i).getName().equals(client.getName()) && clientList.get(i).getAccInfo().getAccNUmber().equals(client.getAccInfo().getAccNUmber())){
                return clientList;
            }
        }
        return null;
    ...
    //메서드를 정의하시오.
    List<Client> accClientInfo(Client client){
        for (int i = 0; i < clientList.size(); i++){
            if (clientList.get(i).getAccInfo().getBalance() > client.getAccInfo().getBalance()){
                return clientList;
            }
        }
                return null;
